...
export function createRunCommand(factory: Factory): Command {
  ...
  command.addCommand(
    createOperationListCommand(factory, {
      ...
      query: (options) =>
        ({
          spaceId:
            typeof options.space === "string" ? options.space : undefined,
          runtimeId:
      ...
      actingSubaccountId: (options) =>
        typeof options.subaccount === "string" ? options.subaccount : undefined,
    }),
  );
  command.addCommand(
  ...
  command.addCommand(
    createRunViewerCommand(factory, "live", "Create a live viewer URL"),
  );
  command.addCommand(
    createRunViewerCommand(
      factory,
      "recording",
      "Create a recording viewer URL",
    ),
  );
  return command;
}
  ...
  command.addCommand(
    createSseCommand(factory, {
      ...
      query: (options) => ({
        ...
        severity:
          typeof options.severity === "string" ? options.severity : undefined,
        invocationId:
          typeof options.invocation === "string"
            ? options.invocation
            : undefined,
...
function createRunActivityListCommand(factory: Factory): Command {
  return addOutputFlags(
    addPaginationFlags(
      new Command("list")
        .description("List run activities")
        .argument("<runId>"),
    )
      .option("--view <view>", "Activity view")
      .option("--type <type>", "Filter by activity type")
    ...
    async (
      ...
      await requestOperationAndPrint(
        factory,
        "runs.activity.list",
        await buildOperationInput("runs.activity.list", options, {
          pathParams: { runId },
          query: {
            view: options.view,
            type: options.type,
  ...
  command.addCommand(
    createSseCommand(factory, {
      ...
      query: (options) => ({
        ...
        contextId:
          typeof options.contextId === "string" ? options.contextId : undefined,
        limit: typeof options.limit === "number" ? options.limit : undefined,
        cursor: typeof options.cursor === "string" ? options.cursor : undefined,
      }),
    }),
...
function createRunEventsListCommand(factory: Factory): Command {
  return addOutputFlags(
    addPaginationFlags(
      new Command("list").description("List run events").argument("<runId>"),
    )
      .option("--type <type>", "Filter by event type")
      .option("--status <status>", "Filter by event status")
      .option("--page-id <id>", "Filter by page id")
    ...
    async (
      ...
      await requestOperationAndPrint(
        factory,
        "runs.events.list",
        await buildOperationInput("runs.events.list", options, {
          pathParams: { runId },
          query: {
            type: options.type,
  ...
  return addOutputFlags(
    ...
    async (
      ...
      await requestOperationAndPrint(
        factory,
        "runs.files.list",
        await buildOperationInput("runs.files.list", options, {
          pathParams: { runId },
          query: {
            type: options.type,
            limit: options.limit,
...
function createRunInvocationsCommand(factory: Factory): Command {
  const command = addOutputFlags(
    addPaginationFlags(
      new Command("invocations")
        .description("List run invocations")
        .argument("<runId>"),
    )
  ...
  command.action(
    async (
      ...
      await requestOperationAndPrint(
        ...
        await buildOperationInput("runs.invocations.list", options, {
          pathParams: { runId },
          query: {
            status: options.status,
...
function createRunInvocationCommand(factory: Factory): Command {
  const command = new Command("invocation").description(
    "Inspect run invocations",
  );
  command.addCommand(
    addOutputFlags(
      new Command("get")
      ...
      async (
        ...
        await requestOperationAndPrint(
          ...
          await buildOperationInput("runs.invocations.get", options, {
...
function createRunViewerCommand(
  ...
  return createOperationJsonBodyCommand(factory, {
    ...
    body: async (_args, options) => {
      ...
      } as CliOperationJsonBody<"runs.live"> &
        CliOperationJsonBody<"runs.recording">;
    },
  });
}
...
function createSseCommand(
  ...
  let command = new Command(config.name)
    .description(config.description)
    .argument("<runId>");
  command = config.configure(command);
  return command.action(
    async (runId: string, options: Record<string, unknown>) => {
...
async function renderSseStream(
  factory: Factory,
...
function formatRunEvent(value: unknown): string {
  ...
  const time =
    typeof event.time === "string"
      ...
      : typeof event.createdAt === "string"
        ? event.createdAt
        : "";
  const label = [event.category, event.type]
  ...
  const suffix =
    event.data && Object.keys(event.data as object).length > 0
      ...
      : "";
  return (
    [time, label || "event"].filter(Boolean).join(" ") + status + name + suffix
  );
}
