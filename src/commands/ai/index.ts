...
export function createAiCommand(factory: Factory): Command {
  const command = new Command("ai").description(
    ...
    createOperationListCommand(factory, {
      ...
      configure: (cmd) =>
        cmd
          ...
          .option(
            "--engine <engine>",
            "Filter by engine: stagehand or browserUse",
          ),
      query: (options) =>
        ({
          provider:
          ...
          managed:
            typeof options.managed === "string"
              ? parseBooleanFlag(options.managed)
              : undefined,
          engine:
            typeof options.engine === "string" ? options.engine : undefined,
...
function createAiCredentialsCommand(factory: Factory): Command {
  const command = new Command("credentials").description(
    "Manage BYOK AI credentials",
  );
  command.addCommand(
    createOperationListCommand(factory, {
  ...
  command.addCommand(
    createAiCredentialWriteCommand(factory, "create", "ai.credentials.create"),
  );
  command.addCommand(
    createAiCredentialWriteCommand(factory, "patch", "ai.credentials.update", [
      "credentialId",
    ]),
  );
  command.addCommand(
...
function createAiCredentialWriteCommand(
  ...
  return createOperationJsonBodyCommand(factory, {
    ...
    name,
    description:
      name === "create" ? "Create an AI credential" : "Update an AI credential",
    argNames,
    configure: (cmd) =>
      cmd
        ...
        .option(
          "--subaccount-id <id>",
          "Subaccount scope when using a parent/org key",
        ),
    ...
    actingSubaccountId:
      operationId === "ai.credentials.create"
        ? (_args, options) =>
            typeof options.subaccountId === "string"
