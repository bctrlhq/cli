...
export function createBrowserExtensionCommand(factory: Factory): Command {
  const command = new Command("browser-extension").description(
  ...
  command.addCommand(
    addOutputFlags(
      new Command("upload")
        ...
        .option(
          "--subaccount-id <id>",
          "Create under a subaccount when using a parent/org key",
        ),
    ).action(
      async (
        path: string,
        options: { name?: string; subaccountId?: string } & OutputFlags,
      ) => {
        const file = await readBlob(path);
        const result = await uploadOperationFile(
          factory,
  ...
  command.addCommand(
    createOperationJsonBodyCommand(factory, {
      ...
      actingSubaccountId: (_args, options) =>
        typeof options.subaccountId === "string"
          ? options.subaccountId
          : undefined,
    }),
  ...
  command.addCommand(
    createOperationJsonBodyCommand(factory, {
      ...
      configure: (cmd) => cmd.option("--name <name>", "Display name"),
      body: async (_args, options) => {
        return {
          name: options.name,
        } as CliOperationJsonBody<"browser-extensions.update">;
      },
    }),
  );
  command.addCommand(
